function diversificaPortafogliHRP
    ...
    mu = mean(R) * 252; % Rendimento annualizzato
    Sigma = cov(R) * 252; % Matrice di covarianza annualizzata
    ...
    disp(table({'Giornaliero'; 'Annualizzato'}, [riskMV; riskMV * sqrt(252)], [retMV; retMV * 252], 'VariableNames', {'Periodo', 'Rischio', 'Rendimento'}));
    disp('Rischio e rendimento del portafoglio HRP:');
    disp(table({'Giornaliero'; 'Annualizzato'}, [riskHRP; riskHRP * sqrt(252)], [retHRP; retHRP * 252], 'VariableNames', {'Periodo', 'Rischio', 'Rendimento'}));
